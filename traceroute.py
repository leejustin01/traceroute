...
class IcmpHelperLibrary:
    ...
    class IcmpPacket:
        ...
        # class variables that store the ping statistics
        totalPings = 0
        successfulPings = 0
        rtts = []
        timeouts = 0
        ...
        def __validateIcmpReplyPacketWithOriginalPingData(self, icmpReplyPacket):
            ...
            receivedSeqNum = icmpReplyPacket.getIcmpSequenceNumber()
            expectedSeqNum = self.getPacketSequenceNumber()
            seqNumValid = receivedSeqNum == expectedSeqNum
            icmpReplyPacket.setSeqNum_isValid(seqNumValid)
            icmpReplyPacket.setIsValidResponse(seqNumValid and icmpReplyPacket.isValidResponse())
            if (self.__DEBUG_IcmpPacket):
                print(f"[DEBUG] Sequence Number: Expected={expectedSeqNum}, Actual={receivedSeqNum}, Valid={seqNumValid}")
                
            # Validate Identifier
            receivedID = icmpReplyPacket.getIcmpIdentifier()
            expectedID = self.getPacketIdentifier()
            identifierValid = receivedID == expectedID
            icmpReplyPacket.setPacketID_isValid(identifierValid)
            icmpReplyPacket.setIsValidResponse(identifierValid and icmpReplyPacket.isValidResponse())
            if (self.__DEBUG_IcmpPacket):
                print(f"[DEBUG] Identifier: Expected={expectedID}, Actual={receivedID}, Valid={identifierValid}")
                
            # Validate Raw Data
            receivedData = icmpReplyPacket.getIcmpData()
            expectedData = self.getDataRaw()
            dataValid = receivedData == expectedData
            icmpReplyPacket.setRawData_isValid(dataValid)
            icmpReplyPacket.setIsValidResponse(dataValid and icmpReplyPacket.isValidResponse())
            if (self.__DEBUG_IcmpPacket):
                print(f"[DEBUG] Raw Data: Expected={expectedData}, Actual={receivedData}, Valid={dataValid}")
                
            if (self.__DEBUG_IcmpPacket):
                print(f"[DEBUG] Final Packet Validation Result: {icmpReplyPacket.isValidResponse()}")
        ...
        def sendEchoRequest(self):
            ...
            try:
                ...
                IcmpHelperLibrary.IcmpPacket.totalPings += 1
                timeLeft = 30
                ...
                if whatReady[0] == []:  # Timeout
                    ...
                    IcmpHelperLibrary.IcmpPacket.timeouts += 1
                ...
                if timeLeft <= 0:
                    ...
                    IcmpHelperLibrary.IcmpPacket.rtts.append(rtt)
                    
                    if icmpType == 11 or icmpType == 3:
                        
                        # dictionary for the icmp errors
                        ICMP_CODES = {
                        ...
                        # get the message using the (type, code) tuple as a key
                        error_message = ICMP_CODES.get((icmpType, icmpCode), "Unknown ICMP Type/Code")
                        ...
                        IcmpHelperLibrary.IcmpPacket.successfulPings += 1
    ...
    class IcmpPacket_EchoReply:
        ...
        __DEBUG_EchoReply = False
        
        # variables for validation
        __seqNum_isValid = False
        __packetID_isValid = False
        __rawData_isValid = False
        ...
        # getter functions for the validation variables
        def getSeqNum_isValid(self):
            return self.__seqNum_isValid
        
        def getPacketID_isValid(self):
            return self.__packetID_isValid
        
        def getRawData_isValid(self):
            return self.__rawData_isValid
        ...
        # setters for validation variables
        def setSeqNum_isValid(self, booleanValue):
            self.__seqNum_isValid = booleanValue
            
        def setPacketID_isValid(self, booleanValue):
            self.__packetID_isValid = booleanValue
            
        def setRawData_isValid(self, booleanValue):
            self.__rawData_isValid = booleanValue
        ...
        #                                                                                                              #
        # ############################################################################################################ #
        def printResultToConsole(self, ttl, timeReceived, addr, sentPacket):
            ...
            # validate self and print any mismatches
            if not self.isValidResponse():
                print("  ICMP Response Error:")
                if not self.getSeqNum_isValid():
                    print(f"    Invalid Sequence Number: Expected={sentPacket.getPacketSequenceNumber()}, Received={self.getIcmpSequenceNumber()}")
                if not self.getPacketID_isValid():
                    print(f"    Invalid Packet Identifier: Expected={sentPacket.getPacketIdentifier()}, Received={self.getIcmpIdentifier()}")
                if not self.getRawData_isValid():
                    print(f"    Invalid Raw Data: Expected={sentPacket.getDataRaw()}, Received={self.getIcmpData()}")
    ...
    # Reference for how traceroute works: https://networklessons.com/cisco/ccna-routing-switching-icnd1-100-105/traceroute
    # Bulk of the code for this function is taken from the skeleton code function: __sendIcmpEchoRequest(self, host)
    def __sendIcmpTraceRoute(self, host):
        ...
        for ttl in range(1, MAX_HOPS+1):
            ...
            icmpPacket.buildPacket_echoRequest(packetIdentifier, packetSequenceNumber)  # Build ICMP for IP payload
            icmpPacket.setIcmpTarget(host)
            icmpPacket.setTtl(ttl) # set the time to live to the current iteration of the for loop so we can send a packet to each router leading up to the destination
            icmpPacket.sendEchoRequest()                                                # Build IP
    ...
    def sendPing(self, targetHost):
        ...
        IcmpHelperLibrary.IcmpPacket.totalPings = 0
        IcmpHelperLibrary.IcmpPacket.successfulPings = 0
        IcmpHelperLibrary.IcmpPacket.rtts = []
        IcmpHelperLibrary.IcmpPacket.timeouts = 0
        self.__sendIcmpEchoRequest(targetHost)
        IcmpHelperLibrary.printPingStatistics()
    ...
    # function for compiling and printing the statistics of the pings
    def printPingStatistics():
        if IcmpHelperLibrary.IcmpPacket.totalPings > 0:
            packetLoss = (IcmpHelperLibrary.IcmpPacket.timeouts / IcmpHelperLibrary.IcmpPacket.totalPings) * 100
            minRTT = maxRTT = IcmpHelperLibrary.IcmpPacket.rtts[0]
            avgRTT = 0
            for rtt in IcmpHelperLibrary.IcmpPacket.rtts: # find the min and max rtt and calculate the sum so we can find the average
                minRTT = min(minRTT, rtt)
                maxRTT = max(maxRTT, rtt)
                avgRTT += rtt
            avgRTT /= len(IcmpHelperLibrary.IcmpPacket.rtts)
            print("\nPing Statistics:")
            print(f"    Pings Sent: {IcmpHelperLibrary.IcmpPacket.totalPings}")
            print(f"    Pings Received: {IcmpHelperLibrary.IcmpPacket.successfulPings}")
